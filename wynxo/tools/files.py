...
class ReadFile(Tool):
    ...
    def _fit_context(self, body: str, note: str, window: list, args,
        ...
        raw_allowance = max(MIN_READ_TOKENS, int(self.context_left * READ_SHARE))
        allowance = min(self.context_left, raw_allowance)
    ...
    async def run(self, args: MultiEditInput) -> ToolResult:
        ...
        for i, edit in enumerate(args.edits, 1):
            ...
            if count == 0:
                ...
                return ToolResult.failure(f"edit {i}: old_text not found in {rel}. {hint} No edits were applied.")
        ...
        stats = diff_stats(diff)
        return ToolResult.success(
            f"applied {len(args.edits)} edit(s) to {rel}{note}", display=diff,
            path=rel, changed=before != text, edits=len(args.edits),
            additions=stats.additions, deletions=stats.deletions,
            sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
        )
