...
def finish(python: Path, venv_dir: Path, launcher: Path | None,
           path_was_updated: bool) -> None:
    print()
    print(S.green(S.bold("  wynxo is installed.")))
    print()

    if launcher is not None and on_path(launcher.parent):
        command = S.bold("wynxo")
        print(f"    Run it:  {command}")
...
def main() -> int:
    ...
    parser.add_argument("--no-link", action="store_true",
                        help="do not put a `wynxo` command on PATH")
    ...
    install_wynxo(
        python,
        into_user=(python == Path(sys.executable)
